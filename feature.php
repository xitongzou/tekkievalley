...
if ( isset($_GET['a']) && $_GET['a'] == 'delete' ) {
    ...
    if ( $VID ) {
        $sql = "DELETE FROM feature_req WHERE VID = '" .mysql_real_escape_string($VID). "'";
        $conn->execute($sql);
...
function constructQuery()
{
    ...
    if ( isset($_POST['search_videos']) ) {
        ...
        if ( $option['username'] != '' ) {
            ...
            $UID            = ( $UID ) ? $UID : 0;
            $query_option[] = $query_add. " v.UID = '" .mysql_real_escape_string($UID). "'";
            $query_add      = " AND";
        }

        if ( $option['title'] != '' ) {
            $query_option[] = $query_add. " v.title LIKE '%" .mysql_real_escape_string($option['title']). "%'";
            $query_add      = " AND";
        }

        if ( $option['description'] != '' ) {
            $query_option[] = $query_add. " v.description LIKE '%" .mysql_real_escape_string($option['description']). "%'";
            $query_add      = " AND";
        }
